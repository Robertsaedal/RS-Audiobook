    ...
    <template v-if="!state.isLoading">
      ...
      <footer class="px-10 pb-16 space-y-12 max-w-xl mx-auto w-full z-20">
        <div class="space-y-4">
          ...
          >
            <div 
              class="absolute inset-y-0 left-0 bg-yellow-500 shadow-[0_0_12px_rgba(234,179,8,0.5)] transition-all duration-150" 
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-neutral-900/20 border border-white/5 rounded-[24px] p-5 flex flex-col items-center gap-4">
            ...
            <div class="flex items-center justify-between w-full px-2">
              <button @click="setPlaybackRate(Math.max(0.5, state.playbackRate - 0.1))" class="p-1 text-neutral-600 hover:text-white transition-colors">
              ...
              <button @click="setPlaybackRate(Math.min(2.5, state.playbackRate + 0.1))" class="p-1 text-neutral-600 hover:text-white transition-colors">
          ...
          <div class="bg-neutral-900/20 border border-white/5 rounded-[24px] p-5 flex flex-col items-center gap-4">
            ...
            <div class="flex items-center justify-between w-full px-2">
              <button @click="state.sleepChapters = Math.max(0, state.sleepChapters - 1)" class="p-1 text-neutral-600 hover:text-white transition-colors">
                <Minus :size="14" stroke-width="2.5" />
              </button>
              ...
              <button @click="state.sleepChapters = Math.min(10, state.sleepChapters + 1)" class="p-1 text-neutral-600 hover:text-white transition-colors">
                <Plus :size="14" stroke-width="2.5" />
              </button>
            </div>
          </div>
        ...
        <div class="flex-1 overflow-y-auto p-10 space-y-12 max-w-2xl mx-auto w-full no-scrollbar pb-32">
          ...
          <div class="space-y-4">
            <div class="flex items-center gap-2 text-neutral-700">
              <Book :size="14" />
              <span class="text-[9px] font-black uppercase tracking-[0.4em]">Artifact Summary</span>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8 border-t border-white/5 pt-12">
            <div v-for="row in infoRows" :key="row.label" class="flex flex-col gap-2">
              <div class="flex items-center gap-2 text-neutral-500">
                ...
                <span class="text-[9px] font-black uppercase tracking-[0.4em]">{{ row.label }}</span>
              </div>
              
              <template v-if="row.isClickable">
                <button 
                  @click="handleSeriesClick"
                  class="text-left text-sm font-bold text-white hover:text-purple-400 transition-colors cursor-pointer w-fit py-1 px-3 bg-purple-600/10 border border-purple-500/20 rounded-full active:scale-95"
                >
                  {{ row.value }}
                </button>
              </template>
      ...
      <div v-if="showChapters" class="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[150] flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-8 no-scrollbar max-w-2xl mx-auto w-full pb-32">
          <button 
            ...
            class="w-full flex items-center justify-between p-6 rounded-[24px] mb-3 transition-all border border-transparent group text-left"
            :class="currentChapterIndex === i ? 'bg-purple-600/10 border-purple-500/20' : 'hover:bg-white/5'"
          >
