...
// Real-time socket updates (Simulated via ABSService pattern)
onMounted(() => {
  ...
  props.absService.onLibraryUpdate(() => {
    // Ideally refetch series data here if the socket indicates a series-level change
    // For now, we assume standard library updates refresh context
  });
});

onUnmounted(() => {
  // Socket cleanup handled by ABSService instance in Library.vue
});
</script>

<template>
  <div class="flex flex-col min-h-full animate-fade-in relative bg-[#0d0d0d]">
    <!-- Immersive Hero Section (50vh Height) -->
    <section class="relative h-[50vh] -mx-8 md:-mx-16 lg:-mx-20 -mt-12 overflow-hidden mb-16 group">
      <!-- Blurred Background (40px Blur / Opacity 0.3) -->
      <div 
      ...
      <div class="absolute top-8 left-8 right-8 flex justify-between items-center z-30">
        ...
        <div class="flex items-center gap-4">
          ...
          <button 
            @click="scanLibrary"
            class="group p-3 rounded-full bg-black/40 backdrop-blur-xl border border-white/5 hover:border-purple-500/30 transition-all active:scale-95"
          >
            <RotateCw 
              :size="18" 
              class="text-neutral-500 group-hover:text-purple-500 transition-colors" 
              :class="{ 'animate-spin text-purple-500': isScanning }" 
            />
          </button>
        </div>
      </div>

      <!-- Immersive Metadata Content -->
    ...
    <section class="space-y-12 pb-40">
      <div class="flex items-center justify-between px-2">
        ...
        <div class="text-[9px] font-black uppercase tracking-widest text-neutral-600">
          Ascending Order
        </div>
      </div>
