...
const displaySequence = computed(() => {
  if (props.fallbackSequence !== undefined && props.fallbackSequence !== null) return props.fallbackSequence;
...
const updateStates = async () => {
  ...
  if (isDownloaded.value) {
    const offlineUrl = await OfflineManager.getCoverUrl(props.item.id);
    if (offlineUrl) localCover.value = offlineUrl;
  }
};
...
<template>
  ...
  >
    <div 
      class="relative w-full aspect-[2/3] bg-neutral-950 rounded-xl overflow-hidden border transition-all duration-500 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] group-hover:scale-[1.04] shrink-0"
    ...
    >
      ...
      <div v-if="!isFinished" class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity z-20 flex items-center justify-center pointer-events-none">
      ...
      <div v-if="!hideProgress && !isFinished && progressPercentage > 0" class="absolute bottom-3 left-3 right-3 z-30 flex flex-col pointer-events-none">
         <div class="relative w-full h-1 bg-purple-950/40 backdrop-blur-sm rounded-full overflow-hidden">
            <div class="h-full bg-purple-500 rounded-full transition-all duration-300 shadow-[0_0_10px_#A855F7]" :style="{ width: progressPercentage + '%', backgroundColor: accentColor }" />
         </div>
      </div>
    </div>
    
    <div v-if="showMetadata" class="mt-3 px-1 flex flex-col gap-1 w-full min-h-[3.5em]">
      <h3 class="text-[11px] font-black text-white uppercase tracking-tight leading-[1.3] line-clamp-2 w-full group-hover:text-purple-400 transition-colors">
