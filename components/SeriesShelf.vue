...
const ITEMS_PER_FETCH = 24; // Increased for better grid filling
    ...
    <div ref="scrollContainerRef" class="flex-1 overflow-y-auto custom-scrollbar px-2 pb-40 relative">
      ...
      <div ref="sentinelRef" class="h-40 w-full flex flex-col items-center justify-center mt-12 mb-20 gap-6">
        <div v-if="isLoading" class="flex flex-col items-center gap-4">
          ...
          <p class="text-[8px] font-black uppercase tracking-[0.4em] text-neutral-700">Indexing Series...</p>
        </div>
        ...
        <div v-else-if="duplicateWallDetected && hasMore" class="flex flex-col items-center gap-4 animate-fade-in">
           ...
           <div class="flex items-center gap-4">
              <button @click="reset()" class="flex items-center gap-3 px-6 py-3 bg-neutral-900/60 border border-white/5 rounded-full text-[9px] font-black uppercase tracking-[0.4em] text-neutral-400 hover:text-purple-400 hover:border-purple-500/20 transition-all active:scale-95">
                <RotateCw :size="14" />
                <span>Force Re-Sync</span>
              </button>
              <button @click="fetchMoreSeries()" class="flex items-center gap-3 px-6 py-3 bg-neutral-900/60 border border-white/5 rounded-full text-[9px] font-black uppercase tracking-[0.4em] text-neutral-400 hover:text-purple-400 hover:border-purple-500/20 transition-all active:scale-95">
                <FastForward :size="14" />
...
.animate-stagger-fade {
  animation: stagger-fade 0.5s cubic-bezier(0.2, 0.8, 0.2, 1) forwards;
  opacity: 0;
}
