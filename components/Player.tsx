...
import { ChevronDown, Play, Pause, SkipBack, SkipForward, Timer, Info, X, Activity, Plus, Minus, AlertCircle, RotateCcw, RotateCw } from 'lucide-react';
...
const Player: React.FC<PlayerProps> = ({ auth, item, onBack }) => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (!details) throw new Error("Unable to fetch item metadata.");
  ...
  const handleAdjustSleep = (mins: number) => {
    setSleepSeconds(prev => Math.max(0, prev + (mins * 60)));
  };
  ...
  if (error) return (
    <div className="h-[100dvh] flex flex-col items-center justify-center bg-black p-10 gap-6 text-center">
      ...
      <button onClick={() => window.location.reload()} className="px-10 py-4 bg-purple-600 rounded-full font-black uppercase text-[10px] tracking-[0.3em] active:scale-95 transition-all shadow-lg">Retry Portal</button>
    ...
    <div className="h-[100dvh] w-full bg-black text-white flex flex-col relative overflow-hidden font-sans select-none">
      ...
      <header className="px-8 pt-10 pb-4 flex justify-between items-center z-20 shrink-0">
        ...
        <button onClick={() => setShowChapters(true)} className="flex items-center gap-2 bg-neutral-900/60 px-4 py-2 rounded-full border border-white/5 group active:scale-95 transition-all max-w-[50%]">
          <span className="text-[9px] font-black uppercase tracking-[0.2em] text-purple-500 truncate">{currentChapter?.title || 'Index'}</span>
      ...
      <div className="flex-1 flex flex-col items-center justify-between px-8 py-4 overflow-hidden">
        <div className="flex-1 w-full flex items-center justify-center mb-6 relative">
          <button onClick={() => setShowInfo(true)} className="aspect-square w-full max-w-[320px] relative group active:scale-95 transition-all shadow-2xl">
        ...
        <div className="w-full space-y-6 shrink-0 pb-8 max-w-xl mx-auto">
          <div className="flex flex-col items-center justify-center text-center w-full px-4 space-y-2">
            {item.media.metadata.seriesName && (
              <div className="px-3 py-1 rounded-full bg-neutral-900 border border-white/10 mb-1">
                <span className="text-[9px] font-black uppercase tracking-widest text-neutral-400">{item.media.metadata.seriesName} #{item.media.metadata.sequence}</span>
              </div>
            )}
          ...
          <div className="flex items-center justify-center gap-6 md:gap-10 w-full py-4">
            <button onClick={() => { if(audioRef.current) audioRef.current.currentTime -= 15; }} className="p-4 text-neutral-400 hover:text-white transition-all active:scale-75 flex flex-col items-center gap-1">
              <RotateCcw size={28} />
              <span className="text-[8px] font-black">15s</span>
            </button>
            ...
            <button onClick={() => { if(audioRef.current) audioRef.current.currentTime += 30; }} className="p-4 text-neutral-400 hover:text-white transition-all active:scale-75 flex flex-col items-center gap-1">
              <RotateCw size={28} />
          ...
          <div className="grid grid-cols-2 gap-4 pb-2">
            <div className="bg-neutral-900/40 rounded-[32px] p-5 flex flex-col items-center justify-center border border-white/5 backdrop-blur-md">
              ...
              <div className="flex items-center gap-6">
                <button onClick={() => setPlaybackSpeed(s => Math.max(0.5, s - 0.1))} className="w-10 h-10 flex items-center justify-center bg-black/40 rounded-full text-neutral-400 hover:text-white active:scale-90 transition-all border border-white/5"><Minus size={16} /></button>
                <span className="text-xs font-black text-purple-500 tracking-widest font-mono">{playbackSpeed.toFixed(1)}x</span>
                <button onClick={() => setPlaybackSpeed(s => Math.min(2.5, s + 0.1))} className="w-10 h-10 flex items-center justify-center bg-black/40 rounded-full text-neutral-400 hover:text-white active:scale-90 transition-all border border-white/5"><Plus size={16} /></button>
              </div>
            </div>

            <div className={`bg-neutral-900/40 rounded-[32px] p-5 flex flex-col items-center justify-center border transition-all backdrop-blur-md ${sleepSeconds > 0 ? 'border-purple-600/40 bg-purple-600/5' : 'border-white/5'}`}>
              <div className="flex items-center gap-2 mb-2">
                <Timer size={10} className={sleepSeconds > 0 ? 'text-purple-500' : 'text-neutral-700'} />
                <span className={`text-[8px] font-black uppercase tracking-[0.2em] ${sleepSeconds > 0 ? 'text-purple-500' : 'text-neutral-700'}`}>Sleep Timer</span>
              </div>
              <div className="flex items-center gap-6">
                <button onClick={() => handleAdjustSleep(-5)} className="w-10 h-10 flex items-center justify-center bg-black/40 rounded-full text-neutral-400 hover:text-white active:scale-90 transition-all border border-white/5"><Minus size={16} /></button>
                <div className="flex flex-col items-center justify-center min-w-[50px]">
                  <span className="text-xs font-black text-white tracking-widest font-mono">{sleepSeconds === 0 ? 'OFF' : formatTime(sleepSeconds)}</span>
                  {sleepSeconds > 0 && <span className="text-[7px] font-black text-purple-500 uppercase tracking-widest mt-0.5">Suspension</span>}
                </div>
                <button onClick={() => handleAdjustSleep(15)} className="w-10 h-10 flex items-center justify-center bg-black/40 rounded-full text-neutral-400 hover:text-white active:scale-90 transition-all border border-white/5"><Plus size={16} /></button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showChapters && (
        <div className="fixed inset-0 z-[100] animate-fade-in flex flex-col bg-black">
          ...
          <div className="flex-1 overflow-y-auto p-4 no-scrollbar max-w-2xl mx-auto w-full">
            {chapters.map((ch, i) => (
              <button key={i} onClick={() => { if(audioRef.current) { audioRef.current.currentTime = ch.start; setCurrentTime(ch.start); setShowChapters(false); }}} className={`w-full flex items-center justify-between p-6 rounded-[32px] mb-3 transition-all ${currentChapterIndex === i ? 'bg-purple-600/10 border border-purple-600/30' : 'hover:bg-neutral-900 border border-transparent'}`}>
          ...
          <div className="bg-neutral-900 w-full max-w-xl rounded-[56px] border border-white/10 overflow-hidden flex flex-col max-h-[85vh]">
             <div className="p-10 space-y-8 overflow-y-auto no-scrollbar">
                ...
                <div className="space-y-4">
                   ...
                   <p className="text-[13px] text-neutral-400 font-medium leading-relaxed uppercase tracking-wide">{item.media.metadata.description || 'No archive description available.'}</p>
                </div>
